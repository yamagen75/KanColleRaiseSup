...
function print_port() {
	...
	mc[0] += dpnla.tmprep(2,tc,tp[1]);	var ia = 0;
	for(i = 0;i < 13;i++){
		ra = new Array();
		switch(i){
			case 1:
	...
	if (lockeditem_ids.length > 0) {
		...
		if(ca > 0) hb += tc[3];
		hb += '</div>';	ra = [lockeditem_count,0,leveling_slotitem,dpnla.tmpagemk(ky,cd),hb,levelmax_slotitem];
		ra[1] = items - lockeditem_count;
