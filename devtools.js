...
function push_fleet_status(tp, deck) {
	...
	for (var i = 0, ship, s_id; ship = $ship_list[s_id = deck.api_ship[i]]; ++i) {
		...
		rt[0] += dpnla.tmprep(2,ra,tp[0][1]);		rt[1] += dpnla.tmprep(2,ra,tp[1][1]);
...
function print_port() {
	...
	var tp = new Array();		var tb = new Array();		var td = new Array();
	...
	for(i = 1;i < 9;i++){
		ra[i] = material_name(i);
	}
	mc[0] += dpnla.tmprep(2,ra,tp[1]);
	for(i = 0;i < 12;i++){
		ra = new Array();
		switch(i){
	...
	if ($logbook.length > 0) {
		...
		ht += tp[2];
		dpnla.tmpviw(0,'t51_5',ht);
	}else{
		dpnla.tmpviw(0,'t51_5','');
	}
	
	tp = dpnla.tmpget('tp1_2');		tb = dpnla.tmpget('tp1_3');
	var ha = '';	ht = '';
	if(mc[1] != '') ht += dpnla.tmprep(0,mc[1],td[1]);
	ht += tp[3];
	//
	// 遂行中任務を一覧表示する.
	...
	if(ndocks > 0 || repairs > 0 || kdocks > 0){
		...
		if(ndocks > 0){
			...
			ra[0] = dpnla.tmprep(0,'入渠中：0',tp[13]);
		}
		if(kdocks > 0){
	...
	mc[3] += dpnla.tmprep(2,me,tp[5]) + mc[4] + mc[0];
	dpnla.tmpviw(0,'t01_1',mc[3]);
	dpnla.tabinit(0,'t11');		dpnla.tabdef('t11');
	if(ndocks > 0){
...
function on_battle(json) {
	...
	if (d.api_formation) {
		...
		$battle_info = fmt;
		var rg = new RegExp('/',"g");		fmt = fmt.replace(rg,' / ');
		fmt = fmt.replace('+',' + ');		fmt = fmt.replace('敵','敵 ');
	}
	var req = [];
